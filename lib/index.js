...
function encodeSingle(type, arg) {
  if (type === "address") {
    ...
    return new Buffer(new BN(arg).toArray('be', 32));
  } else if (type.startsWith("int")) {
    var size = parseTypeN(type);
    if ((size % 8) || (size < 8) || (size > 256))
      throw new Error("Invalid int<N> width: " + size);
    return new Buffer(toTwos(new BN(arg)).toArray('be', 32));
  }
...
function decodeSingle(type, arg) {
  if (type === "address") {
    ...
    if ((size % 8) || (size < 8) || (size > 256))
      throw new Error("Invalid uint<N> width: " + size);
    // FIXME: return the appropriate size as specified
    return new BN(arg.slice(0, 32), 16, 'be');
  } else if (type.startsWith("int")) {
    var size = parseTypeN(type);
    if ((size % 8) || (size < 8) || (size > 256))
      throw new Error("Invalid uint<N> width: " + size);
    // FIXME: return the appropriate size as specified
    return fromTwos(new BN(arg.slice(0, 32), 16, 'be'));
  }
  // FIXME: support ureal<N>x<M> and real<N>x<M>
