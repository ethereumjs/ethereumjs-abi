...
function encodeSingle (type, arg) {
  ...
  if (type === 'address') {
    ...
    return encodeSingle('uint256', new BN(arg).imul(new BN(1).iushln(size[1])))
  } else if (type.startsWith('fixed')) {
    size = parseTypeNxM(type)

    return encodeSingle('int256', new BN(arg).imul(new BN(1).iushln(size[1])))
  }
...
function decodeSingle (type, arg) {
  ...
  if (type === 'address') {
    ...
    return num.div(new BN(1).iushln(size[1]))
  } else if (type.startsWith('fixed')) {
    size = parseTypeNxM(type)
