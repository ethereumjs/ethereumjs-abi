...
function decodeSingle(type, arg) {
  if (type === "address") {
    ...
    var size = bnToInteger(decodeSingle("uint256", arg.slice(0, 32)));
...
ABI.prototype.solidityPack = function(types, values) {
  ...
  for (var i = 0; i < types.length; i++) {
    ...
    if (type === "bytes") {
      ...
      ret.push(utf8.encode(value));
    } else if (type === "address") {
      ret.push(utils.pad(value, 20));
    } else if (type.startsWith("uint")) {
      var size = parseTypeN(type);
      // FIXME: validate size
      ret.push(utils.pad(value, size / 8));
    } else {
      // FIXME: support all other types
      throw new Error("Unsupported type");
    }
  }
  return new Buffer.concat(ret);
}
...
ABI.prototype.soliditySHA256 = function(types, values) {
  ...
  return sha256.update(this.solidityPack(types, values)).digest();
}

ABI.prototype.solidityRIPEMD160 = function(types, values) {
  var ripemd160 = crypto.createHash('RSA-RIPEMD160');
  return utils.pad(ripemd160.update(this.solidityPack(types, values)).digest(), 32);
}
