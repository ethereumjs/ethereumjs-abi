...
function decodeSingle (type, arg) {
  ...
  if (type === 'address') {
    ...
    ret = []
    type = type.slice(0, type.lastIndexOf('['))
    var countOffset = parseTypeArray(type) === 'dynamic' ? 32 : 0
    for (i = 0; i < size; i++) {
      var decoded = decodeSingle(type, arg.slice(offset))
      ret.push(decoded)
      offset += countOffset + 32 * (Array.isArray(decoded) ? decoded.length : 1)
    }
    return ret
  } else if (type === 'bytes') {
    size = decodeSingle('uint256', arg.slice(0, 32)).toNumber()
